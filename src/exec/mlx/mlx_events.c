...
/*   Updated: 2025/01/23 18:08:52 by mcogne--         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	move_mouse_event(int pos_x, int pos_y, t_env *env)
{
	...
	if (!last_pos_y)
		last_pos_y = pos_y;
	if (last_pos_x < pos_y)
		update_player_angle(env, (last_pos_x - pos_x) * MOUSE_SPEED, 0);
	else if (last_pos_x > pos_x)
		update_player_angle(env, (last_pos_x - pos_x) * MOUSE_SPEED, 0);
	//
	if (last_pos_y < pos_y)
		update_player_angle(env, 0, (last_pos_y - pos_y) * MOUSE_SPEED);
	else if (last_pos_y > pos_y)
		update_player_angle(env, 0, (last_pos_y - pos_y) * MOUSE_SPEED);
	last_pos_x = pos_x;
	last_pos_y = pos_y;
	return (0);
