...
/*   Updated: 2025/01/26 15:50:09 by mcogne--         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int *rgb_ceiling)
{
	...
	while (i < 3)
	{
		if ((rgb_floor[i] == -1 && !textures->path_bot) || (rgb_ceiling[i] == -1
				&& !textures->path_top))
			return (1);
		if (rgb_floor[i] < -1 || rgb_floor[i] > 255)
			return (1);
		if (rgb_ceiling[i] < -1 || rgb_ceiling[i] > 255)
			return (1);
		i++;
	}
	return (0);
